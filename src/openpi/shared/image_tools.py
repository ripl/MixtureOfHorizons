...
def resize_torch(
    ...
    """PyTorch version of direct resize. Resizes an image to a target height and width directly
    without padding. If the image is float32, it must be in the range [-1, 1].
    ...
    Returns:
        Resized tensor with same shape format as input
    """
    # Check if input is in channels-last format [*b, h, w, c] or channels-first [*b, c, h, w]
    if images.shape[-1] <= 4:  # Assume channels-last format
        channels_last = True
    ...
    resized_images = F.interpolate(
        images, size=(height, width), mode=mode, align_corners=False if mode == "bilinear" else None
    )
    ...
    # Convert back to original format if needed
    if channels_last:
        resized_images = resized_images.permute(0, 2, 3, 1)  # [b, c, h, w] -> [b, h, w, c]
        if batch_size == 1 and images.shape[0] == 1:
            resized_images = resized_images.squeeze(0)  # Remove batch dimension if it was added
